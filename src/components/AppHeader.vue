...
<template>
  <!-- HEADER: фиксированный и САМЫЙ верхний -->
  <header
    class="fixed left-0 top-0 z-[1000] w-full bg-[#02030A] shadow-[0_8px_28px_rgba(0,0,0,0.24)]"
  >
    <div class="mx-auto max-w-[1720px] px-4 sm:px-6 lg:px-10">
      <div class="relative flex h-[74px] items-center justify-between lg:h-[88px]">
        ...
        <!-- Mobile burger: выше всего в хедере -->
        <button
          type="button"
          class="absolute right-0 top-1/2 z-[1001] inline-flex h-11 w-11 shrink-0 -translate-y-1/2 items-center justify-center rounded-full border border-white/15 bg-white/[0.03] text-white transition hover:bg-white/5 active:scale-[0.98] lg:hidden"
  ...
  <!-- Mobile drawer: должен перекрывать вообще всё -->
  <Teleport to="body">
    <Transition
    ...
    >
      ...
      >
        <!-- Backdrop -->
        <div class="absolute inset-0 bg-black/70" @click="closeMobileMenu" />
        ...
        >
          <div
            v-if="isMobileMenuOpen"
            id="mobile-menu"
            class="absolute right-0 top-0 h-full w-[86%] max-w-[360px] bg-[#060712] shadow-[-14px_0_40px_rgba(0,0,0,0.45)]"
          >
            <div class="flex h-[74px] items-center justify-between px-5">
              <a
                href="/"
              ...
              >
                <span class="text-[18px] font-semibold leading-none tracking-[-0.02em]">MoonAI</span>
              </a>

              <button
                type="button"
                class="inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-white/[0.03] text-white transition hover:bg-white/5 active:scale-[0.98]"
            ...
            <div class="px-6 pb-6 pt-2">
              <nav class="flex flex-col gap-2" aria-label="Мобильная навигация">
                <a
                  ...
                  :key="item.href"
                  :href="item.href"
                  class="rounded-2xl px-4 py-3 text-[18px] leading-[1.2] tracking-[-0.02em] transition"
                  :class="
                    isMobileItemActive(item.href)
                      ? 'bg-white/10 font-semibold text-white'
                      : 'font-medium text-[#C5C8DA] hover:bg-white/[0.06] hover:text-white'
                  "
                  @click="handleMenuItemClick"
                >
                  {{ item.label }}
